    ...
    try {
      ...
      if (payDebtData.paymentType === "ticket" && payDebtData.ticketId) {
        ...
        if (ticketSnap.exists()) {
          ...
          // يصبح مدفوع فقط إذا كان المبلغ المدفوع يساوي المبلغ المستحق تماماً
      ...
      const successMessage =
        payDebtData.paymentType === "general"
          ? "تم خصم المبلغ من رصيد المستخدم بنجاح!"
      ...
      setError("حدث خطأ أثناء دفع المبلغ");
  ...
  return (
    ...
    >
      ...
      >
        {selectedUser && (
          <div className="space-y-2 text-sm">
            ...
            <p>{t("id")}: {selectedUser.id}</p>
            <p>{t("email")}: {selectedUser.email}</p>
            <p>{t("role")}: {selectedUser.role === "admin" ? "مدير" : "وكيل"}</p>
            <p>{t("ticketCount")}: {selectedUser.ticketCount}</p>
            <p>
              {t("totalDebt")}:{" "}
              {getFormattedBalance(selectedUser.balance, currentUser?.preferredCurrency || "USD")}
            </p>
            <p>
              {t("partialPayments")}:{" "}
              {getFormattedBalance(
      ...
      <Modal
        ...
        title={t("manageUserBalance")}
      >
        {balanceUser && (
          <div className="space-y-3">
            <div className="text-sm text-gray-700 mb-4">
              <p>{t("user")}: {balanceUser.name}</p>
              <p>
                {t("currentBalance")}:{" "}
                {getFormattedBalance(
            ...
            >
              <option value="set">{t("setNewBalance")}</option>
              <option value="add">{t("add2balance")}</option>
              <option value="subtract">{t("subtractFromBalance")}</option>
            </select>
        ...
        {balanceUser && (
          <div className="space-y-3">
            <div className="text-sm text-gray-700 mb-4">
              ...
              <p>
                {t("debt")}{" "}
                {getFormattedBalance(balanceUser.balance || 0, currentUser?.preferredCurrency || "USD")}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm">{t("generalDiscount")}</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    ...
                    className="radio radio-primary"
                  />
                  <span className="text-sm">{t("payTicket")}</span>
                </label>
              </div>
            </div>
